class WorldCusine::API
  ...
  def self.get_area(strarea)
    ...
    area_response["area"].each {|area_hash| WorldCusine::Area.new(area_hash)}
  ...
  def self.get_area_meals(input)
    ...
    meals_response["meal"].each {|meal_hash| WorldCusine::Meal.new(meal_hash)}
  ...
  def self.get_meal_info(idMeal)
    ...
    meal_info_response["info"].each {|info_hash| WorldCusine::Meal.meal_details(info_hash)}
